...
public class NeuralNetwork : MonoBehaviour
{
    ...
    private int inputNodesNb, hiddenNodesNb, outputNodesNb;
    ...
    public void Initialize(int inputNodeCount, int hiddenNodeCount, int outputNodeCount)
    {
    ...
    public List<List<double>> TestNetwork(List<(List<double>, List<double>)> pattern)
    {
        ...
        foreach (var p in pattern)
        {
            ...
            List<double> desiredOutput = p.Item2;
            text.Add($"input = {string.Join(" / ", input)} - desired output = {string.Join(" / ", desiredOutput)} --> actual output = {string.Join(" / ", output)}");
        }

        for (int i = text.Count; i > 0; i--)
        {
